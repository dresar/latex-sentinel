...
import { Droplets, Waves, Thermometer, ShieldCheck, Plus } from "lucide-react";
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <SensorCard title="Nilai pH" value={latestPh} unit="pH" icon={Droplets} variant="primary" subtitle="Indikator utama" />
          ...
          <SensorCard title="Suhu" value={latestTemp} unit="°C" icon={Thermometer} variant="primary" subtitle="Suhu lingkungan" />
          <SensorCard title="Status Kualitas" value={latestClassification.status.split(" ")[0]} unit="" icon={ShieldCheck} variant={latestClassification.color} subtitle={latestClassification.status} />
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="rounded-xl border bg-card p-5">
            <h3 className="text-sm font-semibold text-card-foreground mb-4">Tren pH & TDS</h3>
            <ResponsiveContainer width="100%" height={250}>
              <LineChart data={timeData}>
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                <XAxis dataKey="time" tick={{ fontSize: 10, fill: "hsl(var(--muted-foreground))" }} interval="preserveStartEnd" />
                <YAxis yAxisId="ph" domain={[4, 10]} tick={{ fontSize: 10, fill: "hsl(var(--muted-foreground))" }} />
                ...
                <Line yAxisId="ph" type="monotone" dataKey="ph" stroke="hsl(168, 80%, 42%)" strokeWidth={2} dot={false} name="pH" />
                ...
                <tbody>
                  {measurements.slice(0, 5).map((m) => (
                    <tr key={m.id} className="border-b border-border/50 last:border-0">
                      ...
                      <td className="py-3"><StatusBadge status={m.status} color={statusToColor(m.status)} /></td>
